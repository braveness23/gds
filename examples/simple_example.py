...
def simple_file_example():
    ...
    aubio = AubioOnsetNode(
        method="complex", hop_size=512, threshold=0.3, event_bus=event_bus
    )
...
def simple_microphone_example():
    ...
    source = ALSASourceNode(
        device="default", sample_rate=48000, channels=1, buffer_size=1024
    )
    ...
    threshold = ThresholdDetectorNode(
        threshold_db=-15, min_duration_ms=10, event_bus=event_bus
    )
