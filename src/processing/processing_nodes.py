...
class HighPassFilterNode(AudioNode):
    ...
    def _init_filter(self, sample_rate: int, channels: int):
        ...
        if normalized_cutoff >= 1.0:
            raise ValueError(
                f"Cutoff frequency {self.cutoff_freq}Hz is above Nyquist "
        ...
        self.logger.info(
            f"Initialized {self.filter_type} filter: "
    ...
    def process(self, buffer: AudioBuffer) -> Optional[AudioBuffer]:
        ...
        if buffer.channels == 1:
            # Mono processing
            filtered, self.zi = signal.sosfilt(self.sos, buffer.samples, zi=self.zi)
        else:
            ...
            for ch in range(buffer.channels):
                filtered[:, ch], self.zi[:, ch, :] = signal.sosfilt(
                    self.sos, buffer.samples[:, ch], zi=self.zi[:, ch, :]
                )
