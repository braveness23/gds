    ...
    def _load_mqtt_config(self) -> Dict[str, Any]:
        ...
        return {
            "broker": self.config.get(
                ...
                self.config.get("output.mqtt.broker", "localhost")
            ),
            "port": self.config.get(
                "remote_config.mqtt.port",
                self.config.get("output.mqtt.port", 1883)
            ),
            ...
            "base_topic": self.config.get(
                "remote_config.mqtt.base_topic", "gunshot/config"
            ),
    ...
    def _connect(self) -> bool:
        ...
        try:
            ...
            self.client = mqtt.Client(
                client_id=f"gds_config_{self.node_id}_{int(time.time())}"
            )
