...
class ConfigManager:
    ...
    def apply_changes(
        ...
        if not force:
            ...
            needs_confirmation = (
                validation.risk_level in (ConfigRiskLevel.CRITICAL, ConfigRiskLevel.HIGH)
            )
        else:
            validation = None
            needs_test = False
        ...
        try:
            ...
            if needs_confirmation:
                self.logger.info(
                    f"Waiting {self.rollback_timeout}s for health confirmation..."
                )
            ...
            if self.state_path.exists():
                ...
                if "pending_change" in state:
                    ...
                    if previous_config:
                        ...
                        self.logger.info(
                            f"Restored pending change {self.pending_change.change_id}"
                        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            } if self.last_change_result else None,
