...
def test_bme280(address=0x76, duration=30):
    ...
    try:
        ...
        def on_data(data: EnvironmentalData):
            ...
            print(
                f"Temperature: {data.temperature:6.2f}°C | "
        ...
        if readings:
            ...
            temp_spread = max(r.temperature for r in readings) - min(
                r.temperature for r in readings
            )
            hum_spread = max(r.humidity for r in readings) - min(
            ...
            if temp_spread < 0.5 and hum_spread < 2:
                ...
                print(
                    "⚠️  Sensor readings show variation (normal for changing conditions)"
                )
...
def test_dht(gpio_pin=4, sensor_type="DHT22", duration=30):
    ...
    try:
        ...
        if stats["readings_taken"] > 0:
            error_rate = stats["read_errors"] / (
                stats["readings_taken"] + stats["read_errors"]
            )
        ...
        print("❌ DHT library not installed")
        print("\nInstall with:")
...
def main():
    ...
    parser.add_argument(
        "--check-i2c", action="store_true", help="Check I2C bus and scan for devices"
    )
    parser.add_argument("--test-bme280", action="store_true", help="Test BME280 sensor")
    parser.add_argument("--test-dht22", action="store_true", help="Test DHT22 sensor")
    parser.add_argument("--test-dht11", action="store_true", help="Test DHT11 sensor")
    parser.add_argument(
        "--compare", action="store_true", help="Compare multiple sensors"
    )
    parser.add_argument(
        "--gpio", type=int, default=4, help="GPIO pin for DHT sensor (default: 4)"
    )
    parser.add_argument(
        "--i2c-address",
        type=str,
