...
class TrilaterationEngine:
    ...
    def update_speed_of_sound(self, temperature: float, humidity: float = 50.0):
        ...
        print(f"[Trilateration] Speed of sound updated to {self.speed_of_sound:.1f} m/s "
              f"for {temperature}°C")
    ...
    def trilaterate(self, detections: List[Detection]) -> Optional[TriangulationResult]:
        ...
        """
        if len(detections) < 3:
            print(f"[Trilateration] Need at least 3 detections, got {len(detections)}")
            return None
        ...
        if geometry_score < 0.1:
            print(f"[Trilateration] Poor geometry (score: {geometry_score:.2f}), skipping")
            return None
        ...
        try:
            ...
            print(f"[Trilateration] Solver failed: {e}")
            return None
    ...
    def _solve_position(self, positions: np.ndarray, distance_diffs: np.ndarray) -> Tuple[np.ndarray, float]:
        """
    ...
    def _evaluate_geometry(self, positions: np.ndarray) -> float:
        ...
        if len(positions) >= 3:
            ...
            try:
                ...
                # Fallback: use simple spread measure
    ...
    def _classify_event(self, time_window: float, positions: np.ndarray, 
        ...
        # Expected time for sound to travel max distance
    ...
    def _calculate_confidence(self, detections: List[Detection], 
                             geometry_score: float, residual: float,
...
class TrilaterationServer:
    ...
    def connect(self):
        ...
        print(f"[TrilaterationServer] Connecting to {self.broker}:{self.port}...")
        ...
        print(f"[TrilaterationServer] Server started")
        print(f"  Time window: {self.time_window}s (allows events up to ~{self.time_window * self.engine.speed_of_sound / 1000:.1f}km away)")
        print(f"  Min nodes: {self.min_nodes}")
        print(f"  Speed of sound: {self.engine.speed_of_sound} m/s")
    
    def _on_connect(self, client, userdata, flags, rc):
        """Callback when connected."""
        if rc == 0:
            self.connected = True
            print(f"[TrilaterationServer] Connected to MQTT broker")
            
            # Subscribe to all detection topics
            self.client.subscribe("gunshot/detections", qos=1)
            self.client.subscribe("gunshot/+/detections", qos=1)
            
            print(f"[TrilaterationServer] Subscribed to detection topics")
        else:
            print(f"[TrilaterationServer] Connection failed with code {rc}")
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            ...
            print(f"[TrilaterationServer] Detection from {detection.node_id} "
                  f"at {detection.timestamp:.6f}s (buffer: {len(self.detection_buffer)})")
        
        except Exception as e:
            print(f"[TrilaterationServer] Error processing message: {e}")
    
    def _processing_loop(self):
        """Background loop to process detections."""
        print(f"[TrilaterationServer] Processing loop started")
        ...
        while self.running:
            try:
                ...
                print(f"[TrilaterationServer] Error in processing loop: {e}")
    ...
    def _process_group(self, detections: List[Detection]):
        ...
        if result:
            ...
            print(f"\n{'='*60}")
            print(f"✅ TRILATERATION SUCCESS")
            print(f"{'='*60}")
            print(f"Event Type: {result.event_type}")
            print(f"Location: ({result.latitude:.6f}, {result.longitude:.6f}, {result.altitude:.1f}m)")
            print(f"Timestamp: {datetime.fromtimestamp(result.timestamp).isoformat()}")
            print(f"Confidence: {result.confidence:.2%}")
            print(f"Nodes: {result.num_nodes} ({', '.join(result.contributing_nodes)})")
            print(f"Time Window: {result.time_window:.3f}s")
            print(f"Geometry Score: {result.geometry_score:.2f}")
            print(f"Residual Error: {result.residual_error:.1f}m")
            print(f"{'='*60}\n")
            ...
            print(f"[TrilaterationServer] Trilateration failed for this group")
    ...
    def _publish_result(self, result: TriangulationResult):
        ...
        print(f"[TrilaterationServer] Published result to {topic}")
    ...
    def disconnect(self):
        ...
        print(f"  Events trilaterated: {self.stats['events_trilaterated']}")
...
def main():
    ...
    parser.add_argument('--time-window', type=float, default=30.0,
